...
/**
 * Pagination is a convenience to simply add limits and offsets to any query Part of the motivation for this is using
 * doobie's typechecker, which fails unexpectedly for H2. H2 reports it requires a VARCHAR for limit and offset, which
 * seems wrong.
 */
trait SQLPagination {
